...
async def cmd_special_buttons(message: types.Message):
    ...
    keyboard.add(types.KeyboardButton(text="Запросить геолокацию",
                                      request_location=True))
    keyboard.add(types.KeyboardButton(text="Запросить контакт",
...
async def without_puree(message: types.Message):
    ...
    button_inl_weather = types.KeyboardButton(text='Погода', callback_data='weather_value')
    button_inl_projects = types.KeyboardButton(text='Проекты', callback_data='projects_value')
    button_inl_humor = types.KeyboardButton(text='Шутки', callback_data='humor_value')
    button_inl_random = types.KeyboardButton(text='Рандом', callback_data='random_value')
    button_inl_nasa_apod_photo = types.KeyboardButton(text='NASA APOD', callback_data='nasa_apod_photo')
    button_inl_nasa_epic_photo = types.KeyboardButton(text='NASA EPIC', callback_data='nasa_epic_photo')

    button_inl_site = types.KeyboardButton(text='Наш сайт', url='http://craft-projects.com')
...
async def send_random_value(call: types.CallbackQuery):
    menu_kb_inl = types.InlineKeyboardMarkup(resize_keyboard=True)
    button_inl_random = types.KeyboardButton(text='Сыграем?', callback_data='random_value')
    menu_kb_inl.add(button_inl_random)

    await call.message.answer(('Число от 0 до 10 было :  ' + str(randint(0, 10))), reply_markup=menu_kb_inl)
...
async def send_humor_value(call: types.CallbackQuery):
    ...
    await call.message.answer(mods.humor(), reply_markup=menu_kb_inl)  # +'Тут скоро будут шутки-прибаутки')
